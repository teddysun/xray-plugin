...
// ControlOnConnSetup 通过 Unix domain socket 将 fd 发送给 Android VPN 服务，
// 调用 VpnService.protect() 防止流量回环。
func ControlOnConnSetup(network string, address string, conn syscall.RawConn) error {
	return conn.Control(func(s uintptr) {
		fd := int(s)
		path := "protect_path"
...
// registerControlFunc 注册 Android VPN fd 保护的 Dialer/Listener Controller
